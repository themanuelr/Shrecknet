...
def _delete_collection(name: str) -> None:
    ...
    try:
        ...
        if hasattr(_client, "delete_collection"):
            ...
            try:
                print (f" --- LAST TRIAL FOR THE DELETING! " + _client)
...
def get_chroma_client():
    return chromadb.HttpClient(host="chromadb", port=8001)
...
async def add_page(session: AsyncSession, page_id: int):
    ...
    print (f" --- LOADING CHARACTERISTICS")
    char_texts = []
    ...
    print (f" --- CHARACTERISTIC LOADED. DOC PARTS: {doc_parts}")
...
async def rebuild_world(session: AsyncSession, world_id: int):
    ...
    print (f" - PAGES ADDED!")
    try:
        print (f" - PERSISTING VECTORDB!")
        _client.persist()
        print (f" - PERSISTED VECTORDB!")
    except AttributeError:
        print (f" - ERROR WHEN PERSISTING!")
        pass
