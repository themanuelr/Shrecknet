...
import { Users2, Bot, BookOpenText, Sparkles, Wand2, Book, Search } from "lucide-react";
...
export default function AgentsGuildhallPage() {
  ...
  const routeTask = (params?.task as string) || "conversational";

  const taskMap: Record<string, { title: string; icon: JSX.Element; task: string; canRebuild: boolean }> = {
    conversational: {
  ...
  const pageInfo = taskMap[routeTask] || taskMap.conversational;
