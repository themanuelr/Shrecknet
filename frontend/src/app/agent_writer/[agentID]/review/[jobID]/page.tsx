...
function AgentBubble({ agent, children, loading = false }: any) {
  ...
  return (
    <div className="flex items-start gap-4 mb-4">
      ...
      <div className="relative">
        <div className="bg-[var(--card)] border border-[var(--primary)] rounded-2xl px-4 py-3 shadow-sm text-[var(--foreground)] max-w-lg text-md space-y-2">
          {children}
        </div>
          ...
          if (data.status === "done") {
            ...
            for (const { base, mergedItems } of fullGroups) {
              ...
              if (base.exists || base.mode === "update") {
                ...
                mergedResult.push({ ...backendPage, autogenerated_content: fullAutogen, isUpdate: true });
              } else {
                const created = pagesMap.get(base.target_page_id || base.name);
                if (created) {
                  mergedResult.push({ ...created, autogenerated_content: combinedAutogen, isUpdate: false });
                }
              }
            }
                ...
                activeGen === idx ? (
                  <div key={idx} className="border border-[var(--border)] rounded-xl p-6 bg-[var(--card)] shadow-sm mb-12">
                    ...
                    <AgentBubble agent={agent}>
                      ...
                      <div className="prose prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: p.autogenerated_content }} />
                    </AgentBubble>
