...
import { getNovelistJob, updateNovelistJob } from "@/app/lib/agentAPI";
...
import { createPage, updatePage } from "@/app/lib/pagesAPI";
              ...
              <>
                ...
                <CreatePageForm
                  ...
                  onSubmit={async (payload) => {
                    ...
                    await updatePage(
                      created.id,
                      { autogenerated_content: novelText, updated_by_agent_id: agentId },
                      token || ""
                    );
                    if (jobID) {
                      try {
                        await updateNovelistJob(jobID as string, { action_needed: "done" }, token || "");
                      } catch (err) {
                        console.error("Failed to update job", err);
                      }
                    }
                    router.push(`/worlds/${world.id}/concept/${selectedConcept.id}/page/${created.id}`);
                  }}
                />
                <div className="flex justify-between mt-4">
                  <button
